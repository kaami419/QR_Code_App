...
const createContent = async (req, res) => {
  try {
    ...
    const newContent = new ContentModel({
      qrCode: qrId,
      contentType,
      contentUrl: `http://192.168.43.85:5000${contentUrl}`, // Full URL including hostname
      text: text ? text : "",
    });
...
const setCurrentContent = async (req, res) => {
  try {
    const { qrCodeId, contentId } = req.body;
    // const userId = req.user.id;

    const qrId = new mongoose.Types.ObjectId(qrCodeId);
    const conId = new mongoose.Types.ObjectId(contentId);
    ...
    qrCode.currentContent = contentId;
